...
public class DBConnection {
	// BufferedReader타입으로 값을 돌려주는 read()메소드 IOException 오류 발생시 넘기기(쓰는 사람이 알아서 처리해라)
	...
	// BufferedWriter타입으로 값을 돌려주는 write()메소드 IOException 오류 발생시 넘기기(쓰는 사람이 알아서 처리해라)
	public static BufferedWriter write() throws IOException{
		File file = new File("movie.txt");	// 파일 위치 불러오기
		FileWriter fw = new FileWriter(file);	// 파일쓰기
