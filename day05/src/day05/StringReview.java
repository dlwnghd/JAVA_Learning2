...
//		String res = String.join("", temp);	// 베열temp에 있는 데이터들 연결하여 하나의 String타입 res에 저장 
...
//		
//		System.out.println("첫번째 o의 위치 : " + target.indexOf("o"));	// 첫번째 o의 위치 출력
//		System.out.println("두번째 o의 위치 : " + target.indexOf("o", target.indexOf("o")+1));	// 두번째 o의 위치 출력
//		String res = target.substring(target.indexOf("o", target.indexOf("o")+1));	// String타입 res는 문자 "o"가 나오는 지점까지 자르기, 두번째 o의 위치부터
//		System.out.println(res);	// res 출력
...
//	      ex) 입력 >> 일공이삼
//	      출력 >> 1023
		System.out.print("한글로 숫자를 입력하세요 >> ");	// 사용자로부터 숫자르 한글로 입력하도록 유도
		String input = sc.nextLine();	// String타입 input을 입력받음
		
		String hangeul = "공일이삼사오육칠팔수";	// String타입 hangeul은 "공일이삼사오육칠팔수" 저장
		String hangeul1 = "영";	// String타입 hangeul1은 "영" 저장
...
//			System.out.println("input.charAt(" + i + ") : " + input.charAt(i));	// input 인덱스의 문자를 return, input.charAt(i) 출력
			if (input.charAt(i) == '영') {	// 만약 '공'이 아닌 '영'을 입력했을 경우
				...
				res += hangeul.indexOf(input.charAt(i));	// res = res + hanguel이라는 문자열에서 input 인덱스의 문자의 index 번호
			}
		}
