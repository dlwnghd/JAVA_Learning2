...
public class ListTest {
	public static void main(String[] args) {
		// <> 안에는 리스트 안에 저장할 요소의 클래스 타입을 써줘야 한다(wrapper타입)
		...
		// get(인덱스) : ArrayList안에 해당 인덱스 요소를 Integer타입으로 return
...
//		System.out.println(ar.get(0));	// ar의 인덱스번호 0번 출력
//		System.out.println(ar.get(1));	// ar의 인덱스번호 1번 출력
//		System.out.println(ar.get(2));	// ar의 인덱스번호 2번 출력
//		System.out.println(ar.get(3));	// ar의 인덱스번호 3번 출력
//		System.out.println(ar.get(4)); // 인덱스 번호가 벗어나면 오류 발생
		
		// 값을 중간에 삽입하기
		// 1번째 인덱스에 100 삽입하기
		ar.add(1,100);	// ar배열의 1번째 배열에 100 삽입, 기존에 1번째 자리에 있던 데이터는 뒤로 밀려남
		System.out.println(ar);	// ar 출력
		...
		// isEmpty() : 배열이 비어있으면 true, 값이 있으면 false를 출력하는 메소드
		System.out.println(ar.isEmpty());	// ar배열이 비어있니?, 맞으면 true, 아니면 false
		
		
		ArrayList<String> names = new ArrayList<String>();	// String[] names = new String();, ArrayList를 만들건데 String타입으로 names라는 객체로 정의
